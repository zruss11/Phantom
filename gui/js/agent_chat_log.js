...
(function () {
  ...
  function setupEventListeners() {
    ...
    // Microphone button for voice transcription (Chat page)
    var chatMicBtn = document.getElementById("chatMicBtn");
    if (chatMicBtn && window.tauriBridge && window.tauriBridge.transcription) {
      var chatRecorder = null;

      // Check if transcription is available and update button state
      window.tauriBridge.transcription.isAvailable().then(function(available) {
        if (!available) {
          chatMicBtn.disabled = true;
          chatMicBtn.title = "Voice input unavailable - login to Codex first";
      ...
      chatMicBtn.addEventListener("click", function() {
        ...
        if (chatRecorder && chatRecorder.isRecording()) {
          chatMicBtn.classList.remove("recording");
          chatMicBtn.classList.add("transcribing");
          chatMicBtn.querySelector("i").className = "fas fa-spinner";
          ...
          chatRecorder.stop()
            ...
            .finally(function() {
              ...
              chatMicBtn.querySelector("i").className = "fas fa-microphone";
              chatRecorder = null;
            });
          return;
        }
        ...
        // Start recording
        chatRecorder = window.tauriBridge.transcription.createRecorder({
          language: "en",
          onRecording: function() {
            chatMicBtn.classList.add("recording");
            chatMicBtn.querySelector("i").className = "fas fa-stop";
          },
          onTranscript: function(text) {
            // Append transcribed text to chat input
            var trimmedText = text.trim();
            if (trimmedText) {
              var currentVal = chatInput.value || "";
              var separator = currentVal.trim() ? " " : "";
              chatInput.value = currentVal + separator + trimmedText;
