    ...
    try {
      if (action === "codex-login") {
        ...
        if (button.dataset.authState === "connected") {
          ...
          sendNotification("Finish Claude sign-in in your browser", "blue");
          ...
          if (claudeAuthState.authenticated) {
            sendNotification("Claude connected", "green");
            tryEnableClaudeUsage();
          } else {
            sendNotification("Claude login not completed", "yellow");
          }
        }
      }
    } catch (err) {
      ...
      if (action === "claude-login") {
        sendNotification(
          "Claude login failed. You can paste a setup token manually.",
          "red",
        );
      } else {
