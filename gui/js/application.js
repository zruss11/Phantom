...
function updateClaudeAuthUI() {
  ...
  document.querySelectorAll('[data-auth-status="claude"]').forEach((node) => {
    if (state.authenticated) {
      if (state.method === "setupToken") {
        node.textContent = "Setup token configured";
      } else if (state.method === "api") {
        node.textContent = "API key configured";
      } else {
    ...
    try {
      if (action === "codex-login") {
        ...
        if (button.dataset.authState === "connected") {
          ...
          const input = document.querySelector('[data-auth-key="claudeSetupToken"]');
          if (input) input.value = "";
          if (currentSettings) {
            currentSettings.claudeSetupToken = "";
          }
          updateClaudeAuthUI();
          sendNotification("Signed out of Claude", "green");
        } else {
          button.textContent = "Waiting for token...";
          button.disabled = true;
          button.classList.add("auth-pending");

          await ipcRenderer.invoke("claudeLogin");

          const token = window.prompt(
            "Paste the Claude setup token generated by `claude setup-token`.",
          );
          if (token && token.trim()) {
            const input = document.querySelector('[data-auth-key="claudeSetupToken"]');
            ...
            if (claudeAuthState.authenticated) {
              sendNotification("Claude setup token saved", "green");
              tryEnableClaudeUsage();
            } else {
              sendNotification("Token saved but auth not verified", "yellow");
            }
          } else {
            sendNotification("Setup token not provided", "yellow");
          }
      ...
      if (action === "codex-login") {
        ...
        button.classList.remove("auth-pending");
        button.disabled = false;
        button.textContent = "Generate setup token";
      }
...
async function getSettings() {
  ...
  document.querySelectorAll("[data-auth-status]").forEach((node) => {
    ...
    if (key === "claude" && settingsPayload.claudeAuthMethod) {
      if (settingsPayload.claudeAuthMethod === "setupToken") {
        node.textContent = "Setup token configured";
      } else if (settingsPayload.claudeAuthMethod === "api") {
        node.textContent = "API key configured";
      } else {
        node.textContent = "Connected";
      }
